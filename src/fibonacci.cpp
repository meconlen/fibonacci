...
boost::multiprecision::mpz_int fibonacci_fast_doubling_memoized_mpz(boost::multiprecision::mpz_int n)
{
    ...
    return util::fast_doubling_mpz_impl(n, memo).first;
}
...
std::pair<boost::multiprecision::mpz_int, boost::multiprecision::mpz_int> fast_doubling_mpz_impl(boost::multiprecision::mpz_int n, std::map<boost::multiprecision::mpz_int, boost::multiprecision::mpz_int> &memo)
{
    if(memo.contains(n) && memo.contains(n+1)) return { memo[n], memo[n+1] };
    mpz_int k = ( n % 2 == 0 ? n/(mpz_int)2 : (n-(mpz_int)1)/(mpz_int)2 );
    auto [f_k, f_k_1] = fast_doubling_mpz_impl(k, memo);
    ...
    if(n % 2 == 0) {
        memo.insert({n, f_n});
        memo.insert({n+1, f_n_1});
        return { f_n, f_n_1 };
    } else {
        memo.insert({n, f_n});
        memo.insert({n+1, f_n_1 + f_n});
        return {f_n_1, f_n_1 + f_n};
    }
}

// BM_FIBONACCI_FAST_DOUBLING_ITERATIVE_MPZ_20000000       80.8 ms         79.8 ms           87
